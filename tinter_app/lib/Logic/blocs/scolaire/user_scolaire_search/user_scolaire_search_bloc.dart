...
class UserScolaireSearchBloc extends Bloc<UserScolaireSearchEvent, UserScolaireSearchState> {
  ...
  UserScolaireSearchBloc({@required this.userScolaireRepository, @required this.authenticationBloc, @required this.matchedMatchesRepository})
  ...
  Stream<UserScolaireSearchState> _mapAssociationRefreshEventToState() async* {
    yield UserScolaireSearchRefreshingState(searchedUsers: (state as UserScolaireSearchLoadSuccessfulState).searchedUsers);
    ...
    try {
      ...
      print(error);
      yield UserScolaireSearchRefreshingFailedState(searchedUsers:
          (state as UserScolaireSearchLoadSuccessfulState).searchedUsers);
    }
    yield UserScolaireSearchLoadSuccessfulState(searchedUsers: searchedUsers);
  }

  void _mapUserScolaireSearchLikeEventToState(UserScolaireSearchLikeEvent userSearchLikeEvent) {
  ...
  void _mapDiscoverMatchIgnoreEventToState(UserScolaireSearchIgnoreEvent userSearchIgnoreEvent) {
    add(UserScolaireSearchChangeStatusEvent(
  ...
  Stream<UserScolaireSearchState> _mapUserScolaireSearchChangeStatusEventEventToState(
    ...
    List<SearchedUserScolaire> oldSearchedUsersScolaire = (state as UserScolaireSearchLoadSuccessfulState).searchedUsers;
    ...
    List<SearchedUserScolaire> newSearchedUsersScolaire = List<SearchedUserScolaire>.from(oldSearchedUsersScolaire);
    ...
    try {
      ...
      print(error);
      yield UserScolaireSearchSavingNewStatusFailedState(searchedUsers: oldSearchedUsersScolaire);
    }
