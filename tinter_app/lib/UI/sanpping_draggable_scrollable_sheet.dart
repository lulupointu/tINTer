...
///     return Scaffold(
///       appBar: AppBar(
///         title: const Text('SnappingDraggableScrollableSheet'),
///       ),
///       body: SizedBox.expand(
///         child: SnappingDraggableScrollableSheet(
...
class SnappingDraggableScrollableSheet extends StatefulWidget {
  ...
  @override
  _SnappingDraggableScrollableSheetState createState() => _SnappingDraggableScrollableSheetState();
}

/// A [Notification] related to the extent, which is the size, and scroll
/// offset, which is the position of the child list, of the
/// [SnappingDraggableScrollableSheet].
///
/// [SnappingDraggableScrollableSheet] widgets notify their ancestors when the size of
...
/// [SnappingDraggableScrollableSheet] widgets. To focus on notifications from the
...
class SnappingDraggableScrollableNotification extends Notification with ViewportNotificationMixin {
  /// Creates a notification that the extent of a [SnappingDraggableScrollableSheet] has
  /// changed.
  ///
  ...
  SnappingDraggableScrollableNotification({
...
/// Manages state between [_SnappingDraggableScrollableSheetState],
/// [_SnappingDraggableScrollableSheetScrollController], and
/// [_SnappingDraggableScrollableSheetScrollPosition].
///
...
class _DraggableSheetExtent {
  ...
  double get currentExtentPixels => currentExtent*availablePixels;

  double get additionalMinExtent => isAtMin ? 0.0 : 1.0;
  double get additionalMaxExtent => isAtMax ? 0.0 : 1.0;

  double get minExtentPixels => minExtent * availablePixels;
  double get maxExtentPixels => availablePixels;

  double get deltaMinExtentPixels => (minExtent - currentExtent) * availablePixels / maxExtent;
  double get deltaMaxExtentPixels => (1 - currentExtent/maxExtent) * availablePixels;
...
class _SnappingDraggableScrollableSheetState extends State<SnappingDraggableScrollableSheet> {
  _SnappingDraggableScrollableSheetScrollController _scrollController;
  ...
  void initState() {
    ...
    _scrollController = _SnappingDraggableScrollableSheetScrollController(extent: _extent);
  }
...
/// by a [SnappingDraggableScrollableSheet].
///
/// If a [SnappingDraggableScrollableSheet] contains content that is exceeds the height
...
/// See also:
///
///  * [_SnappingDraggableScrollableSheetScrollPosition], which manages the positioning logic for
///    this controller.
///  * [PrimaryScrollController], which can be used to establish a
///    [_SnappingDraggableScrollableSheetScrollController] as the primary controller for
///    descendants.
class _SnappingDraggableScrollableSheetScrollController extends ScrollController {
  _SnappingDraggableScrollableSheetScrollController({
  ...
  @override
  _SnappingDraggableScrollableSheetScrollPosition createScrollPosition(
      ScrollPhysics physics,
      ScrollContext context,
      ScrollPosition oldPosition,
      ) {
    return _SnappingDraggableScrollableSheetScrollPosition(
      physics: physics,
      context: context,
...
/// See also:
///
///  * [_SnappingDraggableScrollableSheetScrollController], which uses this as its [ScrollPosition].
class _SnappingDraggableScrollableSheetScrollPosition
  ...
  void goBallistic(double velocity) {
    if (velocity == 0.0 && (listShouldScroll || extent.isAtMax)  ||
    ...
    // If the sheet should stop between the min position and the max position
    // snap it to either min or max pos.
    final double finalPos = extent.currentExtentPixels + simulation.x(double.infinity);
    if (finalPos < extent.maxExtentPixels && finalPos > extent.minExtentPixels) {
      final SpringDescription springDescription =
      SpringDescription.withDampingRatio(mass: 100, stiffness: 100, ratio: 0.001);

      /// pixels needed to go to max sheet or min sheet
      double deltaTargetPixel;

      if (velocity==0) {
        if (-extent.deltaMinExtentPixels > extent.deltaMaxExtentPixels) {
          deltaTargetPixel = extent.deltaMaxExtentPixels;
        } else {
          deltaTargetPixel = extent.deltaMinExtentPixels;
        }
      } else if (velocity < 0) {
          deltaTargetPixel = extent.deltaMinExtentPixels;
      } else {
    ...
    final AnimationController ballisticController = AnimationController.unbounded(
      debugLabel: objectRuntimeType(this, '_SnappingDraggableScrollableSheetPosition'),
      vsync: context.vsync,
    );
    double lastDelta = 0;
    void _tick() {
...
/// A widget that can notify a descendent [SnappingDraggableScrollableSheet] that it
...
class SnappingDraggableScrollableActuator extends StatelessWidget {
  /// Creates a widget that can notify descendent [SnappingDraggableScrollableSheet]s
  /// to reset to their initial position.
  ///
  /// The [child] parameter is required.
  SnappingDraggableScrollableActuator({
    Key key,
    @required this.child,
  }) : super(key: key);

  /// This child's [SnappingDraggableScrollableSheet] descendant will be reset when the
  ...
  /// Notifies any descendant [SnappingDraggableScrollableSheet] that it should reset
  /// to its initial position.
  ///
  /// Returns `true` if a [SnappingDraggableScrollableActuator] is available and
  /// some [SnappingDraggableScrollableSheet] is listening for updates, `false`
  /// otherwise.
...
class _InheritedResetNotifier extends InheritedNotifier<_ResetNotifier> {
  /// Creates an [InheritedNotifier] that the [SnappingDraggableScrollableSheet] will
  ...
  /// Specifies whether the [SnappingDraggableScrollableSheet] should reset to its
  /// initial position.
  ///
